...
class DataPreprocessingToolkit(object):
    ...
    def add_night_price(self, df):
        # Write your code here
        price_per_room = df["accomodation_price"] / df["n_rooms"]
        price_per_room_per_night = price_per_room / df["length_of_stay"]
        df.loc[:, "night_price"] = np.round(price_per_room_per_night, 2)
        return df
    ...
    def map_night_price_to_room_segment_buckets(self, df):  
        # Write your code here
        # Last too low acomodation_price is 1.94, first normal price is 100.0
        night_price_avg = df.loc[df['accomodation_price'] > 2].groupby('room_group_id')['night_price'].mean().round(2).reset_index()
        night_price_avg.rename(columns={'night_price': 'room_night_price'}, inplace=True)
        df = pd.merge(df, night_price_avg, on=['room_group_id'], how='left')
        df['room_night_price'] = df['room_night_price'].fillna(0)
        df.loc[:, 'room_segment'] = df['room_night_price'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
